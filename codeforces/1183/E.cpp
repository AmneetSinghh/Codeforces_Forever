...
while(!q.empty())
{
  ...
  {
  	if(full.size()==k){flag=1;break;}
  	string val=t.substr(0,max(i,0ll))+t.substr(i+1,n);
  	if(full.find(val)!=full.end())continue;
  	else full.insert(val),q.push(val),cost+=(n-val.size());
  	if(full.size()==k){flag=1;break;}
  }
