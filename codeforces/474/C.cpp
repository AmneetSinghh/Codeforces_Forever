...
// }
int x[5],y[5],a[5],b[5],xx[5],yy[5];



void rotate(int idx,int times){
    f(i,times){
            int xi=x[idx]-a[idx], yi=y[idx]-b[idx],xf,yf;
    yf=xi; xf=-1*yi;
    x[idx] = xf+a[idx];
    y[idx] = yf+b[idx];
    }
...
bool is_square(){
    if(dist(0,1) == dist(2,3) && dist(0,2) == dist(1,3) && dist(0,3) == dist(1,2)){
        if(dist(0,1) && dist(0,2) && dist(0,3)){
            if(dist(0,1) == dist(0,2) || dist(0,2) == dist(0,3) || dist(0,1) == dist(0,3)){
                return true;
            }
        } 
    }
...
int res=10000009;
        ...
        for(int j=1;j<=4;j++)      
        {
            ...
            for(int k=1;k<=4;k++)
            {
                ...
                for(int l=1;l<=4;l++)
                {
                    rotate(4-1,l);
                    if(is_square())
                        res=min(i%4+j%4+k%4+l%4,res);
                        x[4-1]=xx[4-1],y[4-1]=yy[4-1];
                }
                x[3-1]=xx[3-1],y[3-1]=yy[3-1];
                
            }
            x[2-1]=xx[2-1],y[2-1]=yy[2-1];
        }
    ...
    if(res==10000009)// this means its not square
        cout<<"-1\n";
    else cout<<res<<"\n";
