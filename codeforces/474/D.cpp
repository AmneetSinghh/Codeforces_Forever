...
{
    if(i<k)dp[i]=((dp[i-1]%mod)+mod)%mod;
    else dp[i]=(((((dp[i-1]%mod)+mod)%mod)+(((dp[i-k]%mod)+mod)%mod))+mod)%mod;
    if(i==1) prefix[i]=((dp[i]%mod)+mod)%mod;
    else prefix[i]=(((((prefix[i-1]%mod)+mod)%mod)+(((dp[i]%mod)+mod)%mod))+mod)%mod;
}
...
F(i,t)
{
    int l,r;
    cin>>l>>r;
    cout<<(((prefix[r]%mod)-(prefix[l-1]%mod))+mod)%mod;tr;
