...
for(int i=1;i<=sqrt(n);i++)
{
    if(n%i==0)
    {
     ...
     {
      for(int j=1;j<=first;j++){// in thtese posiiont we want one baby
               int s=1;
             for(int k=j;k<=n;k+=first)s&=a[k];

                if(s==1){flag=1;break;}
      }
     }

     if(second<n/2 || second==1)
     {
            for(int j=1;j<=second;j++){// in thtese posiiont we want one baby
               int s=1;
             for(int k=j;k<=n;k+=second)s&=a[k];
