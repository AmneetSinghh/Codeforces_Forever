...
int n,arr[505],arr2[505],tmp[505];
vector<int> ans;

void shift(int idx){
    ans.pb(idx);
    tmp[idx] = arr[idx+2] , tmp[idx+1] = arr[idx] , tmp[idx+2] = arr[idx+1];
    for(int j=idx;j<=idx+2;j++) arr[j] = tmp[j];
}
...
for(int i=1;i<=n;i++){
    int idx=i;
    while(arr[idx]!=arr2[i])idx++;// which we have to do bro
    ...
    while(idx!=i){
            ...
            if(idx==i+1)
            {
                ...
                {
                    if(arr[n-2]==arr[n]){// may be equal is this value baby
                        shift(n-2);
                        break;
                    }
