...
void dfs1(int s,int p)
{
  ...
  if(p){
  if(dp[s]<=0)
  	dp[s]=max(dp[s],dp[s]+dp[p]);
  else dp[s]=max(dp[s],dp[p]);
}
...
F(i,n)
{
	if(a[i]){dfs(i,0);start=i;break;}
}
if(!start)F(i,n)cout<<"-1 ";
else
{
	dfs1(start,0);F(i,n)cout<<dp[i]<<" ";
}
