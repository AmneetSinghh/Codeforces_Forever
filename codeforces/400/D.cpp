...
{ 
    ...
    { 
        ...
        { 
            int v = (*i).first; 
            int weight = (*i).second; 
              if (ans[src][v]==-1  || ans[src][v]>ans[src][u]+ weight) 
                { 
                ans[src][v] = ans[src][u]+ weight; 
                pq.push(make_pair(ans[src][v], v)); 
                } 
        } 
    } 
} 
...
for(int i=1;i<=k;i++)
{
        int val;cin>>val;
        for(int j=1;j<=val;j++)
        back[start]=i,cc[i].pb(start),++start;
}
F(i,m)
{
    int u,v,w;
...
F(i,k)F(j,k)if(i==j)ans[i][j]=0; else ans[i][j]=-1;
// apppy dijkstra i think baby
F(i,k)dijkstra(i);
cout<<"Yes\n";
F(i,k)
{
