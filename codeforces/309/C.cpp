...
scanf("%d%d", &n, &m);
    for(int i = 0; i < n; i++){
        scanf("%d", &a[i]);
        for(int j = 0; j < 31; j++){
            if(a[i] & (1 << j)) cnt[j]++;//every interger can be made by 2^ powers so only one t incremnt kro oh ohvaliya powers to banne baby
        }
    }
    ...
    while(l < 31 && r < m){
        if(res >= (1 << b[r])){
            res -= (1 << b[r]);// may the next element we can do with this baby
            r++;
        } else if(cnt[l] > 0){
            res = 1 << l;
            cnt[l]--;
