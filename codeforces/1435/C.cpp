...
void Not_Stable(){
	...
	sort(all(p));
    int vis[n+1]={0};
    int cum=0,ans=inf,head=0;
	for(auto c:p)
	{
      vis[c.second]++;
      if(vis[c.second]==1)++cum;
      if(cum==n)// we get the total al values; cehck the min
      {
                         while(cum==n)
                         {
                         	vis[p[head].second]--;// this id cuts;
                         	if(vis[p[head].second]==0)// this ends;
                         	{
                                   --cum;
                                   ans=min(ans,c.first-p[head].first);
                         	}
                         	++head;/// teke the next minimum value;
                         }
      }
	}
	cout<<ans;
