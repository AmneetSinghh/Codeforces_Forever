...
// .. list is going on
 ...
 int cost[200005],a[200005],b[200005],dp_main[200005][2],ans=0;
vector<int> vc[200005];
 void dfs(int current,int parent)
 {
        if(a[current]==0 && b[current]==1)dp_main[current][0]++;
    else if(a[current]==1 && b[current]==0)dp_main[current][1]++;

    for (int child: vc[current]) 
   {
    if(child!=parent)
    {
            cost[child]=min(cost[current],cost[child]);
            dfs(child,current);
            dp_main[current][0]+=dp_main[child][0];
            dp_main[current][1]+=dp_main[child][1];

             int k=min(dp_main[current][0],dp_main[current][1]);
    dp_main[current][0]-=k,dp_main[current][1]-=k;// this i used baby
    ans+=(k*2*cost[current]);
...
F(i,n){
    ...
    if(a[i]==0 && b[i]==1)first++;
    else if(a[i]==1 && b[i]==0 )second++;
}
F(i,n-1)
{
int u,v;
