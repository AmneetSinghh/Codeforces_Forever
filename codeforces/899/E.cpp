...
struct comp{
    bool operator () (pair<int,int> i,pair<int,int> j){
        if(i.second!=j.second) return i.second>j.second;// secon is the v
        return i.first<j.first;
    }};
int  a[200005],n,len[200005],ans=0;
...
void we_have_choices(){
    ...
    {
     int start=i,count=0;
     while(a[start]==a[i])++start,count++;
     len[i]=count;
     s.insert({i,len[i]}),idx.insert(i);// starting indexes babies
     i=start;
     if(count==0)++i;
    }
