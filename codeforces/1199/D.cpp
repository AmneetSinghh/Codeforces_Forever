...
void Not_Stable() {
    ...
    F(i,n)cin>>a[i];
    cin>>q;
    std::vector<pair<int,ipair>> db;
    F(i,q)
    {
       cin>>t;
       if(t==2){cin>>v;db.pb({t,{i,v}});}
       else{cin>>p>>x,a[p]=x,db.pb({t,{p,0}});}
    }
    ...
    for(int i=0;i<db.size();i++)
    {
    	...
    	auto id=db[i].ss.ff;
    	auto w=db[i].ss.ss;
          if(t==1 && !vis[id])vis[id]=1,a[id]=max(a[id],mx);
          else if(t==2) mx=max(mx,w);
          else{}
    }

    F(i,n)if(!vis[i])a[i]=max(mx,a[i]);
    F(i,n)cout<<a[i]<<" ";
    tr;
