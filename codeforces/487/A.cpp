...
void we_have_choices(){
    int y1,y2,y3,z1,z2,z3,x1,x2,x3;
    cin>>y1>>y2>>y3>>z1>>z2>>z3>>x1>>x2>>x3;

    int mi=100000000000009;
    int range1=y2+200;
    int range2=y3+200;
    for(int i=y2;i<=range1;i++)
    {
    if(i<=z3)continue;
     int total=(i-y2)*x2;
     int kiil=i-z3;
     int needed=(z1/kiil)+((z1%kiil)?1:0);//how many times it will go bro
     if(kiil>z1)needed=1;
     // cout<<needed;tr;
     for(int j=y3;j<=range2;j++)
     {
        int cost=(j-y3)*x3;
        if(j>z2)mi=min(mi,total+cost);
        else
        {
            int ch=(z2-j)*needed;// zmount
            if(y1>ch)mi=min(mi,total+cost);
            else
                mi=min(mi,cost+total+  ((ch-y1)+1)*x1);
     }
